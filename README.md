...
- OCR 使用界面选择的原文语言，需要本机安装对应的 Windows OCR 语言包。选择“自动检测”时，中文、日文或韩文引擎识别出的纯 ASCII 字母文本会使用已安装的英文引擎重新识别；混合文字保留首次结果。英文扫描页也可直接选择 `English`。
...
选中或 OCR 识别出的文本会发送给 Google Translate。不要对密码、token 或其他敏感文本使用划词或 OCR 热键；剪贴板回退也可能暂时改变剪贴板内容。OCR 截图只写入系统临时目录，并会在识别成功或失败后立即删除。
